...
files and calculate monthly average at a specific lat/lon point in Python.
...
for file in sorted(glob.glob('MOD04_3K.A2022*.hdf')):
    ...
    if flag:
        ...
        if (np.isnan(m)):
            ...
            print(m)
            day = file[14:17]
            timebase = datetime.datetime(2022, 1, 1, 0, 0, 0)
            dt = timebase + datetime.timedelta(int(day) - 1)
            l.append([int(dt.month), m])
df = pd.DataFrame(l, columns=['Month', 'Mean'])
print(df)
t = '{0}\n{1}'.format('MOD04_3K 2022 Monthly Average at 40.0E & 10.0N',
                      DATAFIELD_NAME)
plt = df.groupby('Month').mean().plot(title = t)
fig = plt.get_figure()

# Save image.
pngfile = "MOD04_3K.A2022.m.py.png"
fig.savefig(pngfile)
