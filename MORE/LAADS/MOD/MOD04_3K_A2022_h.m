...
% hourly average of multiple LAADS MODIS MOD04_3K L2 swath files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD04_3K_A2022_h
%
...
D = dir(fullfile(thepath, 'MOD04_3K.A2022035*.*.hdf'));
...
for k =1:numel(D)
    ...
    % If you want to plot data at the hour/minute/second level,
    % comment it out to use the raw data.
    [filepath,name,ext] = fileparts(FILE_NAME);
    strs = split(name, ".");
    str = strs(3); % Extract time. (e.g., 0640)
    hour = extractBetween(str, 1, 2);
    time(:) = str2double(hour);

    writematrix(horzcat(time(:), lat(:), lon(:), data(:)), 'outh.csv', ...
                'WriteMode','append');
end

% Read filtered data to calculate average.
A = readtable('outh.csv');
B = varfun(@mean,A,'InputVariables',4,...
           'GroupingVariables',1);
% Draw plot.
f = figure('Name', 'MOD04_3K Hourly Average', 'visible', 'off');
plot(B.Var1, B.mean_Var4);
xtickformat('%d');
xticks(B.Var1);
xlabel('Hour in 2022035');

% Put title.
tstring = {'MOD04_3K Hourly Average in 0~20N 30~50E region';DATAFIELD_NAME};
...
saveas(f, ['MOD04_3K_A2022_h.m.png']);
exit;
