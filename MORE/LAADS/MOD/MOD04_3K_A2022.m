...
% daily average of multiple LAADS MODIS MOD04_3K L2 swath files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD04_3K_A2022
%
...
D = dir(fullfile(thepath, 'MOD04_3K.A2022*.*.hdf'));
...
for k =1:numel(D)
    ...
    day = extractBetween(str,6,8);
    time(:) = str2double(day);

    writematrix(horzcat(time(:), lat(:), lon(:), data(:)), 'out.csv', ...
                'WriteMode','append');
end
...
B = varfun(@mean,A,'InputVariables',4,...
           'GroupingVariables',1);
% Draw plot.
f = figure('Name', 'MOD04_3K Daily Average', 'visible', 'off');
...
% Put title.
tstring = {'MOD04_3K Daily Average at lon=40E lat=10N';DATAFIELD_NAME};
