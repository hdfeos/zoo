...
% visualize LAADS multiple MODIS MOD04 L2 swath files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD04_L2_merge_s
%
...
x = (lon_m > -60.0 & lon_m < -30.0 & lat_m > 15.0 & lat_m < 30.0);
lon_s = lon_m(x);
lat_s = lat_m(x);
data_s = data_m(x);
...
saveas(f, [FILE_NAME '.m.s.png']);
writematrix(horzcat(lat_s(:), lon_s(:), data_s(:)), 'out.s.csv', ...
            'WriteMode','append');
exit;
