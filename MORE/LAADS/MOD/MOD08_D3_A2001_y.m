...
% yearly average of multiple LAADS MODIS MOD08_D3 L3 Grid files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD08_D3_A2001_y
%
...
for k = 1:numel(D)
    ...
    time(:) = year;
    writematrix(horzcat(time, mean(data(:), "omitmissing")), 'outy.csv', ...
                'WriteMode','append');
end
...
B = varfun(@mean, A, 'InputVariables', 2,...
           'GroupingVariables', 1);
% Draw plot.
f = figure('Name', 'MOD08_D3 Yearly Average', 'visible', 'off');
...
% Put title.
tstring = {'MOD08_D3 Yearly Average [0~30N] & [20~60E]';DATAFIELD_NAME};
...
saveas(f, ['MOD08_D3_A2001_y.m.png']);
exit;
