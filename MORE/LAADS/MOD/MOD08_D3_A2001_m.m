...
% monthly average of multiple LAADS MODIS MOD08_D3 L3 Grid files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD08_D3_A2001_m
%
...
for k = 1:numel(D)
    ...
    str_mo = datestr(datevec(datenum(2021, 1, day)), 'mm');
    time = str2double(str_mo);
    writematrix(horzcat(time, mean(data(:), "omitmissing")), 'outm.csv', ...
                'WriteMode','append');
end
...
B = varfun(@mean, A, 'InputVariables', 2,...
           'GroupingVariables', 1);
% Draw plot.
f = figure('Name', 'MOD08_D3 Monthly Average', 'visible', 'off');
plot(B.Var1, B.mean_Var2);
xtickformat('%d');
xticks(B.Var1);
xlabel('Month in 2001');

% Put title.
tstring = {'MOD08_D3 Monthly Average [0~30N] & [20~60E]';DATAFIELD_NAME};
...
saveas(f, ['MOD08_D3_A2001_m.m.png']);
exit;
