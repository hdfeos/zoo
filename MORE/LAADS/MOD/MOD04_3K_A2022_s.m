...
% seasonal average of multiple LAADS MODIS MOD04_3K L2 swath files in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r MOD04_3K_A2022_s
%
...
% Read both 2021 and 2022 data.
D = dir(fullfile(thepath, 'MOD04_3K.A202*.*.hdf'));
...
for k =1:numel(D)
    ...
    strs = split(name, ".");
    str = strs(2); % Extract day. (e.g., A2022036)
    ...
    time(:) = s;
    writematrix(horzcat(time(:), lat(:), lon(:), data(:)), 'outm.csv', ...
                'WriteMode','append');
end

% Read filtered data to calculate average.
A = readtable('outm.csv');
B = varfun(@mean,A,'InputVariables',4,...
           'GroupingVariables',1);
% Draw plot.
f = figure('Name', 'MOD04_3K Seasonal Average', 'visible', 'off');
plot(B.Var1, B.mean_Var4);
xtickformat('%d');
xticks(B.Var1);
xlabel({'Season in 2021/03~2022/02' ; '0=winter 1=spring 2=summer 3=fall'});

% Put title.
tstring = {'MOD04_3K Seasonal Average at lon=40E lat=10N';DATAFIELD_NAME};
...
saveas(f, 'MOD04_3K_A2022_s.m.png');
exit;
