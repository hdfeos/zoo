...
files and calculate yearly average over South Africa region in Python.
...
for fn in sorted(glob.glob("2A23.*.7.HDF")):
    print(fn)
...
# Put titles.
t = "{0}\n{1}".format("2A23 Yearly Average", DATAFIELD_NAME)
t2 = "{0}".format(DATAFIELD_NAME2)

df = pd.DataFrame(_l, columns=["Year", "Mean", "Mean2"])

print(df)

fig, axs = plt.subplots(2, 1, sharex=True)
pl = df.groupby("Year")["Mean"].mean().plot(title=t, ax=axs[0], ylabel=units)
pl.locator_params(integer=True)

# HBB has the same units as freezH.
p2 = df.groupby("Year")["Mean2"].mean().plot(title=t2, color="red",
                                              ax=axs[1], ylabel=units)
p2.locator_params(integer=True)
# Save the plot.
pngfile = "2A23.y.py.png"
fig.savefig(pngfile)
