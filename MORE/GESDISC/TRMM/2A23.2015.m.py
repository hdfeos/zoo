...
files and calculate monthly average over South Africa region in Python.
...
for fn in sorted(glob.glob("2A23.2015*.7.HDF")):
    ...
    if np.isnan(m) or np.isnan(m2):
        ...
        mo = fn[9:11]
        print(mo)
        _l.append([int(mo), m, m2])

    i = i + 1

# Put titles.
t = "{0}\n{1}".format("2A23 2015 Monthly Average", DATAFIELD_NAME)
t2 = "{0}".format(DATAFIELD_NAME2)

df = pd.DataFrame(_l, columns=["Month", "Mean", "Mean2"])

print(df)

fig, axs = plt.subplots(2, 1, sharex=True)
pl = df.groupby("Month")["Mean"].mean().plot(title=t, ax=axs[0], ylabel=units)
# HBB has the same units as freezH.
p2 = df.groupby("Month")["Mean2"].mean().plot(title=t2, color="red",
                                              ax=axs[1], ylabel=units)
p2.locator_params(integer=True)
# Save the plot.
pngfile = "2A23.2015.m.py.png"
fig.savefig(pngfile)
