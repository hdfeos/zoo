...
% 3B42 Grid files and calculate seasonal average over some region in MATLAB.
%
...
%
%    $matlab -nosplash -nodesktop -r TRMM_3B42_Daily_2019_7_nc4_s
%
...
for k = 1:numel(D)
    ...
    s = floor(str2double(mo) / 3);
    if s == 4
        s = 0;
    end
    time(:) = s;
    writematrix(horzcat(time, mean(data_s(:), "omitmissing")), 'out_s.csv', ...
                'WriteMode','append');    
    netcdf.close(file_id);
end

% Read filtered data to calculate average.
A = readtable('out_s.csv');
B = varfun(@mean, A, 'InputVariables', 2,...
           'GroupingVariables', 1);
% Draw plot.
f = figure('Name', '3B42 2019 Seasonal Average', ...
           'visible', 'off');
plot(B.Var1, B.mean_Var2);
xtickformat('%d');
xticks(B.Var1);
xlabel({'Season in 2019' ; ['0=winter 1=spring 2=summer 3=fall']});

% Put title.
ln_strs = split(long_name, 'with');
s = '3B42_2019 Seasonal Average [0~30N] & [20~60E]';
...
saveas(f, ['3B42_Daily.2019.7.nc4.s.m.png']);
exit;
