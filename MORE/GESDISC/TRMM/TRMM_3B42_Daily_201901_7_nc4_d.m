...
% 3B42 Grid files and calculate daily average over some region in MATLAB.
%
...
%
%    $matlab -nosplash -nodesktop -r TRMM_3B42_Daily_201901_7_nc4_d
%
...
D = dir(fullfile(thepath, '3B42_Daily.2019010*.7.nc4'));
...
for k = 1:numel(D)
    ...
    strs = split(name, ".");
    str = strs(2);
    day = extractBetween(str, 7, 8);
    time = str2double(day)
    writematrix(horzcat(time, mean(data_s(:), "omitmissing")), 'out_d.csv', ...
                'WriteMode','append');    
...
B = varfun(@mean, A, 'InputVariables', 2,...
           'GroupingVariables', 1);
% Draw plot.
f = figure('Name', '3B42 2019 Daily Average from Jan 1 to Jan 3', ...
           'visible', 'off');
...
s = '3B42_2019 Daily Average [0~30N] & [20~60E]';
...
saveas(f, ['3B42_Daily.201901.7.nc4.d.m.png']);
exit;
