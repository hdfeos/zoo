...
files and calculate monthly average over some region in Python.
...
Usage: save this script and run
    ...
    $python 3B42_Daily.2019.7.nc4.m.py
...
for fn in sorted(glob.glob("3B42_Daily.20190*.7.nc4")):
    print(fn)
...
t = "{0}\n{1}\n{2}".format(
    "3B42 2019 Monthly Average from Jan to Feb", a[0], "with " + a[1]
)

df = pd.DataFrame(_l, columns=["Month", "Mean"])

print(df)

pl = df.groupby("Month").mean().plot(title=t)
pl.locator_params(integer=True)

# Save the plot.
pngfile = "3B42_Daily.2019.7.nc4.m.py.png"
fig = pl.get_figure()
fig.savefig(pngfile)
