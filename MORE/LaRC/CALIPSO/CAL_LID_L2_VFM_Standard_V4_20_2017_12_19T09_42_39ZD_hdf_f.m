...
%
%  $matlab -nosplash -nodesktop -r CAL_LID_L2_VFM_Standard_V4_20_2017_12_19T09_42_39ZD_hdf_f
%
...
% and the value starts from 1 to 8.
...
% Match color map values.
data = data + 1;

% Create a custom color map for 8 different key values.
cmap = [                                   %  Key      
                    [0.00 0.00 0.00];  ... %  0=invalid
                    [0.00 0.00 1.00];  ... %  1=clear
                    [1.00 1.00 0.00];  ... %  2=cloud
                    [0.00 1.00 0.00];  ... %  3=aerosol
                    [1.00 0.00 0.00];  ... %  4=strato
                    [0.78 0.39 1.00];  ... %  5=surface
                    [0.39 0.20 1.00];  ... %  6=subsurf
                    [0.50 0.50 0.50];  ... %  7=no signal
       ];     


% Pick either descending or ascending plot.
% contourf(lat(1:idx), altitude, rot90(data_at(1:idx,:), 1));
contourf(lat(idx+1:end), altitude, rot90(data(idx+1:end,:), 1));
colormap(cmap);
caxis([1 m]);
...
tstring = {FILE_NAME;'Feature Classification Flag'};
title(tstring, 'Interpreter', 'none', 'FontSize', 16, ...
      'FontWeight','bold');
saveas(f, [FILE_NAME '.f.m.png']);
exit;
