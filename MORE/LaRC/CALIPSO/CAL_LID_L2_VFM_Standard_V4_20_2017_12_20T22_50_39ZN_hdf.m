...
%
%  $matlab -nosplash -nodesktop -r CAL_LID_L2_VFM_Standard_V4_20_2017_12_20T22_50_39ZN_hdf
%
...
% Open the HDF4 File.
FILE_NAME = 'CAL_LID_L2_VFM-Standard-V4-20.2017-12-20T22-50-39ZN.hdf';
...
Y = diff(lat);
d = sign(diff(lat));
idx_u = find(d(1:end-1) < 0 & d(2:end) > 0);
idx_d = find(d(1:end-1) > 0 & d(2:end) < 0);
if(~isempty(idx_u)) 
% Pick either descending or ascending plot.
% contourf(lat(1:idx_u), altitude, rot90(data_at(1:idx_u,:), 1));
  contourf(lat(idx_u+1:end), altitude, rot90(data_at(idx_u+1:end,:), ...
                                           1));
elseif (~isempty(idx_d))
% contourf(lat(1:idx_d), altitude, rot90(data_at(1:idx_d,:), 1));
  contourf(lat(idx_d+1:end), altitude, rot90(data_at(idx_d+1:end,:), ...
                                           1));
else
  contourf(lat, altitude, rot90(data_at, 1));
end
