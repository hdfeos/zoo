...
% Level 2 Vertical Feature Mask Version 4.2 file in MATLAB. 
%
...
%
%  $matlab -nosplash -nodesktop -r CAL_LID_L2_VFM_Standard_V4_20_2017_12_19T09_42_39ZD_hdf_c
%
...
% Open the HDF4 File.
FILE_NAME = 'CAL_LID_L2_VFM-Standard-V4-20.2017-12-19T09-42-39ZD.hdf';
...
% Focus on cloud type (=2) data only. Make the rest fill value (= 0).
data(data_ft > 2) = 0;
data(data_ft < 2) = 0;
% unique(data)
...
data2d = squeeze(data(1:2990, 1166:5515));   % -0.5km to 8.2km
data3d = reshape(data2d, dim, 290, 15);
data = squeeze(data3d(:,:,1));

% Select the 10-12 bits for Cloud Type data. 
...
% Create a custom color map for 8 different Aerosol Type key
% values.
%
...
cmap = [                                   %  Key                   R   G   B
                    [0.00 0.00 0.00];  ... %  0=l.o., transparent [000,000,000]    
                    [0.00 0.00 1.00];  ... %  1=l.o., opaque      [000,000,255]
                    [1.00 1.00 0.00];  ... %  2=tran. strato.     [255,255,000]
                    [0.00 1.00 0.00];  ... %  3=low, broken cum.  [000,255,000]
                    [1.00 0.00 0.00];  ... %  4=altocumulus (t.)  [255,000,000]
                    [0.78 0.39 1.00];  ... %  5=altostratus (o.)  [200,100,255]
                    [0.39 0.20 1.00];  ... %  6=cirrus (t.)       [100,50,255]
                    [0.50 0.50 0.50];  ... %  7=deep convective   [128,128,128]
       ];     

colormap(cmap);
caxis([0 7]); 

% Find the peak latitude point index.
Y = diff(lat);
d = sign(diff(lat));
idx = find(d(1:end-1) < 0 & d(2:end) > 0);

% Pick either descending or ascending plot.
% contourf(lat(1:idx), altitude, rot90(data_at(1:idx,:), 1));
contourf(lat(idx+1:end), altitude, rot90(data_at(idx+1:end,:), 1));
...
% Put colorbar.
y = [0, 1, 2, 3, 4, 5, 6, 7];
h = colorbar('YTickLabel', {'transparent', 'opaque', 'trans. strato', ...
                    'broken cum.', 'altocumuls', ...
                    'altostrao', 'cirrus', 'deep convec.' }, 'YTick', y);

tstring = {FILE_NAME;['Cloud Type  (Bits 10-12) in' ...
