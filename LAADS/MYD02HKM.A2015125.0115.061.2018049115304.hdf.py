"""
This example code illustrates how to access and visualize a LAADS MYD02HKM v6.1
...
Usage:  save this script and run
  ...
  $python MYD02HKM.A2015125.0115.061.2018049115304.hdf.py
...
# $eos2dump -a1 MYD02HKM.A2015125.0115.061.2018049115304.hdf > lat_MYD02HKM.A2015125.0115.061.2018049115304.output
GEO_FILE_NAME = 'lat_MYD02HKM.A2015125.0115.061.2018049115304.output'
latitude = np.genfromtxt(GEO_FILE_NAME, delimiter=',', usecols=[0])
# Find middle location for map.
f = np.floor(latitude.size/2)
lat_m  = latitude[int(f)]
latitude = latitude.reshape(data.shape)

# Run the following command to get the longitude using eos2dump tool [1].
# $eos2dump -a2 MYD02HKM.A2015125.0115.061.2018049115304.hdf > lon_MYD02HKM.A2015125.0115.061.2018049115304.output
GEO_FILE_NAME = 'lon_MYD02HKM.A2015125.0115.061.2018049115304.output'
longitude = np.genfromtxt(GEO_FILE_NAME, delimiter=',', usecols=[0])
# Find middle location for map.
f = np.floor(longitude.size/2)
lon_m = longitude[int(f)]
