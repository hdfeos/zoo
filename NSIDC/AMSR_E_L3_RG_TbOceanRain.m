...
% Reading Data from a Data Field
  GRID_NAM = 'MonthlyRainTotal_GeoGrid';
...
% Translate longitude from 0 to 360 degree to -180 to 180
  lon_offset = -180;
  lon_value = lon_offset + lon_value;

% Since we want to plot the data from -180 to 180 in longitude, and the original data starts from 0 to 360(really means 0 to 180, -180 to 0 in longitude),
% So we need to move the data from -180 to 0 before 0 to 180. That means switching the second half of the array with the first half of the array along
% the dimension of longitude.
  halfx = floor(xdimsize/2);

  data=[data(:, (halfx+1):xdimsize) data(:, 1:halfx)];
...
% Plot the data using contourfm and axesm
  ...
  f = figure('Name','AMSR_E_L3_RainGrid_V06_200206_TbOceanRain','visible','off')

  axesm('MapProjection','eqdcylin','Frame','on','Grid','on', 'MapLatLimit',latlim,'MapLonLimit',lonlim, ...
      'MeridianLabel','on','ParallelLabel','on')
  coast = load('coast.mat');

% Although surfacem more quickly generates the plot than contourfm does,
% contourfm provide more color-smoother plot, so we use contourfm here
% use 'LineStyle', 'none' to eliminate the outlines of color regions
  contourfm(lat,lon,data, 'LineStyle', 'none');
  colormap('Jet');
  caxis([min_data max_data]); 
  h = colorbar('YTick', min_data:50:max_data);
  ...
  saveas(f,'AMSR_E_L3_RainGrid_V06_200206_TbOceanRain.m.jpg');