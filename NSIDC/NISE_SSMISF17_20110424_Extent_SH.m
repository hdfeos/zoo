...
lat1D = load('lat_NISE_SSMISF17_20110424.Southern_Hemisphere.output');
lon1D = load('lon_NISE_SSMISF17_20110424.Southern_Hemisphere.output');
...
% The following will return 11 keys used in the dataset.
z = unique(data);
num_levels = size(z);
...
figure_handle=figure('Name', ...
                     'NISE_SSMISF17_20110424 Southern Hemisphere Extent', ...
                     'visible', 'off');


% Here is the color map used by the MODIS group for the Browse
% images
% Use 11 entries for the values that z = unique(data) returns.
cmap = [
...
% floor(max(max(lat))) is not useful because undefined values extend to
% the opposite pole. Set it to 0.0.
latlim=[floor(min(min(lat))), 0.0];
...
% Surfacem() is faster than contourfm() but it does not support
...
if ishghandle(figure_handle)
  ...
  'NISE_SSMISF17_20110424_25km_SeaIce_Extent_SP_matlab.jpg');
end
