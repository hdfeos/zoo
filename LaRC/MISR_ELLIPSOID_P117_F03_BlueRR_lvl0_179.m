...
lat1D = load('lat_MISR_ELLIPSOID_P117_F03.output');
lon1D = load('lon_MISR_ELLIPSOID_P117_F03.output');

lat3D = reshape(lat1D, ydimsize, xdimsize, nblocks);
lon3D = reshape(lon1D, ydimsize, xdimsize, nblocks);

lat = zeros(nblocks*ydimsize, xdimsize);
lon = zeros(nblocks*ydimsize, xdimsize);

for i = 1:nblocks;
    lat(((i-1)*ydimsize+1):(i*ydimsize),:) = lat3D(:,:,i);
end

for i = 1:nblocks;
    lon(((i-1)*ydimsize+1):(i*ydimsize),:) = lon3D(:,:,i);
end
