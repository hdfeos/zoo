...
% LaRC CALIPSO Lidar Level 2  Vertical Feature Mask 4.20 file in MATLAB. 
%
...
%
% $matlab -nosplash -nodesktop -r CAL_LID_L2_VFM_Standard_V4_20_2019_07_12T04_47_04ZD_hdf
%
...
cmap=[                       %  Key            R   G   B
      [0.00 0.00 0.00];  ... %  0=invalid     [255,255,255]                    
...
% Calculate the middle point of each color interval.
tick_values = linspace(0, 7, length(cmap)+ 1);
tick_labels = (tick_values(1:end-1) + tick_values(2:end)) / 2;

% Set the colorbar tick locations and labels
labels = {'invalid', 'clear', 'cloud', 'aerosol', ...
          'strato', 'surface', 'subsurf', 'no signal'};

% Put colorbar.
h = colorbar('YTickLabel', labels, 'YTick', tick_labels);
caxis([0 7])
