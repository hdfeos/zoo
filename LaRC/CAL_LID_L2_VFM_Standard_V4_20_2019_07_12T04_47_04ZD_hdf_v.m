...
%
%  $matlab -nosplash -nodesktop -r CAL_LID_L2_VFM_Standard_V4_20_2019_07_12T04_47_04ZD_hdf_v
%
...
% Select the 6-7 bits for Ice/Water data. 
data = bitshift(data, -5);
data = bitand(data, 3);

% Subset latitude values that are strictly decreasing or increasing.
...
% Here, we pick a random large number 2990
% since we will subset lat < -18.0 later for actual plotting.
lat = lat(1:2990);
dim = size(lat, 1);
...
% Create a custom color map for 4 different Ice/Water Phase key values.
cmap = [                       %  Key                     R   G   B
                    [1.00 0.00 0.00];  ... %  0=unknown [255,000,000]    
                    [0.00 1.00 0.00];  ... %  1=ice     [000,255,000]
                    [0.00 0.00 1.00];  ... %  2=water   [000,000,255]
                    [0.50 0.50 0.50];  ... %  3=oriented ice [128,128,128]
       ];     

colormap(cmap);
caxis([0 3]); 

% Find the latitude point index of lat=-18.80.
idx = find(lat(1:end-1) < -18.809 & lat(2:end) > -18.81);

% Pick data up to lat = -18.80.
contourf(lat(1:idx), altitude, rot90(data(1:idx,:), 1));
...
% Put colorbar.
y = [0, 1, 2, 3, 4, 5, 6, 7];
h = colorbar('YTickLabel', {'Unknown', 'ice', 'water', ...
                    'oriented ice'},'YTick', y);

tstring = {FILE_NAME;['Ice/Water Phase' ...
                    ' 0 = Unknown    1 = ice    2 = water    3 = ' ...
                    'oriented ice']};
