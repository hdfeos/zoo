...
This example code illustrates how to access and visualize a LaRC CERES ISCCP
D2like GEO Day Edition3A HDF4 file in Python.
...
Usage:  save this script and run
    ...
    $python CER_ISCCP-D2like-GEO_DAY_Edition3A_300300.201612.hdf
...
def run(FILE_NAME):
    ...
    if USE_NETCDF4:    
        ...
        longitude = nc.variables['Longitude - M'][0,:,:].astype(np.float64)
        colatitude = nc.variables['Colatitude - M'][0,:,:].astype(np.float64)
        ...
        lon3D = hdf.select('Longitude - M')
        longitude = lon3D[0,:,:].astype(np.float64)
        lat3D = hdf.select('Colatitude - M')
...
if __name__ == "__main__":
    ...
    hdffile = 'CER_ISCCP-D2like-GEO_DAY_Edition3A_300300.201612.hdf'
    try:
